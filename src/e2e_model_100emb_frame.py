...
class nn:
    ...
    def net(self,x, shapes_batch,softmax_num,is_training, input_dim, is_batchnorm):
        ...
        fc2_r = self.fc_layer(ac2, 100, "fc2r")
        fc2r_bn = self.batch_norm_wrapper_fc(fc2_r, is_training, 'bnr6', is_batchnorm)
        ac2r = tf.nn.relu(fc2r_bn)


        fc3 = self.fc_layer(ac2r,softmax_num,"fc3")
        return fc1, fc2_r, fc3,res1,conv1r,ac1,ac2r
    ...
    def conv_layer(self, bottom, kernel_size,num_channels, stride, depth, name, shape_list):   # n_prev_weight = int(bottom.get_shape()[1])
        ...
        # tf.variable_scope.reuse_variables()  # added by shammur
    ...
    def batch_norm_wrapper_1dcnn(self, inputs, is_training, name, shape_list, is_batchnorm, decay=0.999):
        if is_batchnorm:
            ...
            pop_mean = tf.get_variable(name + 'pop_mean', dtype=tf.float32, initializer=tf.zeros([inputs.get_shape()[-1]]),
                                       trainable=False)
